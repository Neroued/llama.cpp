...
//
// Math + I/O layouts: see chunked_gdn.cuh::stages::chunk_output_config.
// Block / smem total: 4 warps (128 t) with __launch_bounds__(128, 3);
//                     ~48 KB smem at S=128 (q permanent + k_pass / h / v
//                     alias + g).
// Tuning history (V1->V2), ncu numbers, sync audit, K-tile pass design,
// D->A shfl conversion, numerical-stability derivation:
//   chunked/PERF_LOG.md#stage-4-chunk_output-stage_chunk_outputcu
...
// at S<=64 the full k fits in <=16 KB so single pass. See PERF_LOG.md.
// ---------------------------------------------------------------------------
template <int S> struct kernel_dims {
    ...
    static constexpr int K_TILE_PASSES =
        (BT * S * (int) sizeof(float) > 16 * 1024) ? 2 : 1;
    static_assert(K_TILES_S % K_TILE_PASSES == 0,
                  "K_TILE_PASSES must divide K_TILES_S");
...
__global__ void chunk_output_gdn_kernel(const float * __restrict__ q_in,
                                        const float * __restrict__ k_in,
                                        const float * __restrict__ v_new_in,
                                        const float * __restrict__ g_cumsum_in,
                                        const float * __restrict__ h_chunk_in,
    ...
    // q_in / k_in / attn_out are user buffers (batch stride T_total * stride_t
    // for q/k, T_total * H_v * S for attn_out). v_new lives in workspace
    // sized to T (the chunked length), so its base stays on T.
    const int64_t q_base    = ((int64_t) b * T_total + cs) * q_stride_t + qk_head_idx;
    const int64_t k_base    = ((int64_t) b * T_total + cs) * k_stride_t + qk_head_idx;
    const int64_t vn_base   = ((int64_t) b * T + cs) * H_v  * S
                            + (int64_t) h_v  * S;
    const int64_t hc_base   = (((int64_t) b * NT + chunk) * H_v + h_v) * S * S;
    const int64_t out_base  = ((int64_t) b * T_total + cs) * H_v  * S
                            + (int64_t) h_v  * S;
    ...
    gdn::issue_async_load_vec4<BT, S, THREADS>(
        q_view, q_in + q_base, q_stride_t, cl, tid);
    gdn::issue_async_load_vec4<BT, K_PER_PASS, THREADS>(
    ...
    for (int pass = 0; pass < K_TILE_PASSES; ++pass) {
        ...
        for (int kt_local = 0; kt_local < K_TILES_PER_PASS; ++kt_local) {
            ...
            for (int nt = 0; nt < N_TILES_BT; ++nt) {
                ...
                const int row_t  = n_off + lane_g;   // B operand: row=t, but B is K^T so rows are k cols
                const float b0 = k_pass_view.at(row_t, col_t0_l);
                const float b1 = k_pass_view.at(row_t, col_t1_l);
                mma_m16n8k8_tf32(
                    A_strip[nt][0], A_strip[nt][1],
                    A_strip[nt][2], A_strip[nt][3],
                    a0, a1, a2, a3, b0, b1);
            }
        }
        ...
        if (pass + 1 < K_TILE_PASSES) {
            ...
            gdn::issue_async_load_vec4<BT, K_PER_PASS, THREADS>(
                k_pass_view,
                k_in + k_base + (int64_t) (pass + 1) * K_PER_PASS,
                k_stride_t, cl, tid);
            async_copy_commit();
            async_copy_wait_all();
            __syncthreads();
    ...
    // overwrites the bad product with 0.0f; no NaN escapes. See PERF_LOG.md
    // for the full numerical-stability derivation.
    {
    ...
    for (int c = 0; c < N_CHUNKS; ++c) {
        ...
        {
            const float * h_part_gmem =
                h_chunk_in + hc_base + (int64_t) d_chunk_off * S;
            gdn::issue_async_load_vec4<D_CHUNK, S, THREADS>(
                h_part_view, h_part_gmem, /*row_stride=*/(int64_t) S,
                /*cl=*/D_CHUNK, tid);
        }
        async_copy_commit();
        async_copy_wait_all();
        ...
        for (int kt = 0; kt < K_TILES_S; ++kt) {
            ...
            for (int nt = 0; nt < N_TILES_PER_CHUNK; ++nt) {
                ...
                mma_m16n8k8_tf32(
                    D_frag[nt][0], D_frag[nt][1],
                    D_frag[nt][2], D_frag[nt][3],
                    a0, a1, a2, a3, b0, b1);
            }
        }
        ...
        {
            const float * v_part_gmem =
                v_new_in + vn_base + (int64_t) d_chunk_off;
            gdn::issue_async_load_vec4<BT, D_CHUNK, THREADS>(
...
template <int S>
cudaError_t launch_typed(const chunk_output_config & cfg,
                         dim3 grid, gdn::head_map qk_map, int NT, float scale) {
    constexpr int smem_bytes = kernel_dims<S>::SMEM_FLOATS * (int) sizeof(float);

    cudaError_t err = cudaFuncSetAttribute(
        chunk_output_gdn_kernel<S>,
    ...
    const int64_t q_stride_t = (cfg.q_stride_t_floats != 0)
                                ? cfg.q_stride_t_floats
    ...
    chunk_output_gdn_kernel<S><<<grid, block, smem_bytes, cfg.stream>>>(
        cfg.q, cfg.k, cfg.v_new, cfg.g_cumsum, cfg.h_chunk, cfg.attn_out,
        cfg.L, L_total, cfg.H_v, qk_map, q_stride_t, k_stride_t, NT, scale);
...
cudaError_t launch_chunk_output(const chunk_output_config & cfg) {
    detail::stage_validator v{"launch_chunk_output",
    ...
    if (cfg.kda && cfg.Aqk == nullptr) return cudaErrorInvalidValue;
    if (!cfg.kda && cfg.k == nullptr) return cudaErrorInvalidValue;
    ...
    const float   scale  = 1.0f / sqrtf((float) cfg.S);
    GDN_PROPAGATE(v.check_grid(NT, bh));

    const dim3 grid((unsigned) NT, (unsigned) bh, 1);

    switch (cfg.S) {
        case 16:  return launch_typed<16> (cfg, grid, qk_map, (int) NT, scale);
        case 32:  return launch_typed<32> (cfg, grid, qk_map, (int) NT, scale);
        case 64:  return launch_typed<64> (cfg, grid, qk_map, (int) NT, scale);
        case 128: return launch_typed<128>(cfg, grid, qk_map, (int) NT, scale);
